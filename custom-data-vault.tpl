...
VAULT_ZIP_URL="${tpl_vault_zip_file}"
CONSUL_ZIP_URL="${tpl_consul_zip_file}"
...
curl -o /tmp/consul.zip $${CONSUL_ZIP_URL}
...
sudo tee /etc/consul.d/consul-default.json <<EOF
{
  ...
  "log_level": "INFO",
  "ui": true,
  "retry_join": ["provider=azure tag_name=ConsulDC tag_value=consul-demo tenant_id=${tpl_autojoin_tenant_id} client_id=${tpl_autojoin_client_id} subscription_id=${tpl_autojoin_subscription_id} secret_access_key=${tpl_autojoin_secret_access_key}"]
}
EOF
...
curl -o /tmp/vault.zip $${VAULT_ZIP_URL}
